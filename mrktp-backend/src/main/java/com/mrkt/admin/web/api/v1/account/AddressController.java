...
import com.mrkt.admin.service.account.MrktpAddressService;
...
@RestController
@Api(value = "Address API", tags = "Address", description = "Address Management")
@RequestMapping(value = {"v1/account/users/{userId}/addresses", "/account/users/{userId}/addresses"})
@Slf4j
public class AddressController {

    private final MrktpAddressService addressService;
    ...
    public AddressController(MrktpAddressService addressService,
    ...
    @ApiOperation(value = "Create the Address in Market Place platform", response = CreateResponse.class)
    @ResponseStatus(HttpStatus.CREATED)
    @ApiResponses(value = {
            @ApiResponse(code = 201, message = "The Address is created!", response = CreateResponse.class),
            @ApiResponse(code = 400, message = "The request is invalid!"),
            @ApiResponse(code = 500, message = "Server error")})
    public ResponseEntity<CreateResponseData> create(UriComponentsBuilder uriComponentsBuilder, @Valid @RequestBody AddressDto addressDto) throws MrktpNotFoundException {

        ResponseEntity<CreateResponseData> response;
//      groupValidator.validate(sectionDto);
        UserG userG = userService.findResource(addressDto.getUserId());
        CreateResponseData responseData = addressService.create(addressDto);
        Address address = addressService.findResource(responseData.getId());
        address.setUserG(userG);
        userG.getAddresses().add(address);
        userRepository.save(userG);

        UriComponents uriComponents = uriComponentsBuilder.path("/account/users/" + userG.getId() + "/addresses/{id}").buildAndExpand(address.getId());
    ...
    @ApiOperation(value = "Replace the Address in Market Place platform", response = ResponseBase.class)
    @ResponseStatus(HttpStatus.CREATED)
    @ApiResponses(value = {
            @ApiResponse(code = 201, message = "The Address is replaced!", response = ResponseBase.class),
            @ApiResponse(code = 400, message = "The request is invalid!"),
            @ApiResponse(code = 500, message = "Server error")})
    public ResponseEntity<ResponseBase> replace(@PathVariable String id,
                                                @Valid @RequestBody AddressDto addressDto) throws MrktpNotFoundException {
...
//      groupValidator.validate(sectionDto);
        ...
        Address address = addressService.findResource(id);
        address.setUserG(userG);
        userG.getAddresses().add(address);
        userRepository.save(userG);
...
//    }
    ...
    @GetMapping(value = "/{id}", produces = MediaType.APPLICATION_JSON_VALUE,
            consumes = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(value = "Search an  Address in Market Place platform", response = MrktpGroupDto.class)
    public ResponseEntity<AddressDto> findById(@PathVariable String id) throws MrktpNotFoundException {
    ...
    @GetMapping(value = "", produces = MediaType.APPLICATION_JSON_VALUE,
            consumes = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(value = "List of all the Addresses in Market Place platform", response = MasterDataPresentationDto.class)
    public List<MasterDataPresentationDto> findPaginated(@RequestParam(required = false) int page, @RequestParam(required = false) int size, @RequestParam(required = false)
        ...
        return addressService.findAllActive();
    }
